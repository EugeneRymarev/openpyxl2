...
class TestGradientFill:
    ...
    def test_stop_sequence(self, GradientFill, Stop, colors):
        gf = GradientFill(stop=[Stop(colors[0], 0), Stop(colors[1], 0.5)])
...
class TestStop:
    ...
    @pytest.mark.parametrize(
        "position,exception", [(-0.1, ValueError), (1.1, ValueError), (None, TypeError)]
    )
