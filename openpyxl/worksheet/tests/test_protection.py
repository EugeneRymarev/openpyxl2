...
class TestSheetProtection:
    ...
    def test_ctor(self, sheet_protection):
        prot = sheet_protection()
    ...
    def test_other_algorithm(self, sheet_protection):
        ...
        prot = sheet_protection.from_tree(node)
        xml = tostring(prot.to_tree())
        diff = compare_xml(xml, expected)
        assert diff is None, diff

    def test_bool(self, sheet_protection):
        prot = sheet_protection()
...
def test_ctor_with_password(sheet_protection):
    prot = sheet_protection(password="secret")
    assert prot.password == "DAA7"


@pytest.mark.parametrize(
    "password, already_hashed, value",
    [("secret", False, "DAA7"), ("secret", True, "secret")],
)
def test_explicit_password(sheet_protection, password, already_hashed, value):
    prot = sheet_protection()
