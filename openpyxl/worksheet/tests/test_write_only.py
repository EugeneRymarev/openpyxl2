...
def test_path(write_only_worksheet):
    ws = write_only_worksheet
    assert ws.path == "/xl/worksheets/sheetNone.xml"


def test_values_to_rows(write_only_worksheet):
    ws = write_only_worksheet
    ws._max_row = 1
...
def test_append(write_only_worksheet):
    ws = write_only_worksheet
    ws.append([1, "s"])
...
def test_invalid_append(write_only_worksheet, row):
    ws = write_only_worksheet
    with pytest.raises(TypeError):
        ws.append(row)


def test_cannot_save_twice(write_only_worksheet):
    from openpyxl.worksheet._write_only import WorkbookAlreadySaved
...
def test_close(write_only_worksheet):
    ws = write_only_worksheet
    ws.close()
    with open(ws._writer.out, "rb") as src:
        xml = src.read()
...
def test_read_after_closing(write_only_worksheet):
    ws = write_only_worksheet
    ws.close()
    with open(ws._writer.out, "rb") as src:
        xml = src.read()
...
def test_write_only_cell(write_only_worksheet):
    ws = write_only_worksheet
...
def test_hyperlink(write_only_worksheet):
    ws = write_only_worksheet
