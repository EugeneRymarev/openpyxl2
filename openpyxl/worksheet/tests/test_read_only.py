...
class TestReadOnlyWorksheet:
    ...
    def test_read_cell_from_empty_row(
        self, DummyWorkbook, ReadOnlyWorksheet, row, column
    ):
    ...
    def test_pad_rows_before(self, ReadOnlyWorksheet):
        ws = ReadOnlyWorksheet
        rows = ws._cells_by_row(
            min_row=8, max_row=10, min_col=1, max_col=3, values_only=True
        )
        assert list(rows) == [
            (None, None, None),
            (None, None, None),
    ...
    def test_pad_rows_after(self, ReadOnlyWorksheet):
        ws = ReadOnlyWorksheet
        rows = ws._cells_by_row(
            min_row=4, max_row=6, min_col=1, max_col=3, values_only=True
        )
        assert list(rows) == [
            (7, 8, 9),
            (None, None, None),
    ...
    def test_pad_rows_between(self, ReadOnlyWorksheet):
        ws = ReadOnlyWorksheet
        rows = ws._cells_by_row(
            min_row=4, max_row=None, min_col=1, max_col=3, values_only=True
        )
        assert list(rows) == [
            (7, 8, 9),
            (None, None, None),
    ...
    def test_pad_rows_bounded(self, ReadOnlyWorksheet):
        ws = ReadOnlyWorksheet
        rows = ws._cells_by_row(
            min_row=8, max_row=15, min_col=1, max_col=3, values_only=True
        )
        assert list(rows) == [
            (None, None, None),
            (None, None, None),
...
def test_implementation_compatbility(ReadOnlyWorksheet, DummyWorkbook):
    ...
    ro_only = set(["_worksheet_path", "parent", "title", "_shared_strings"])
