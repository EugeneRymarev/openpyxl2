...
from .dimensions import SheetFormatProperties
...
class Worksheet(_WorkbookChild):
    ...
    def _get_cell(self, row, column):
        ...
        if not 0 < row < 1048577:
            msg = f"Row numbers must be between 1 and 1048576. Row number supplied was {row}"
    ...
    def _invalid_row(self, iterable):
        msg = f"Value must be a list, tuple, range or generator, or a dict. Supplied value is {type(iterable)}"
        raise TypeError(msg)
