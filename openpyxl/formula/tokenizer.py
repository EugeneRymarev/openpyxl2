...
class Tokenizer:
    ...
    # Each of these characters, marks the end of an operand token
    TOKEN_ENDERS = ",;}) +-*/^&=><%"

    def __init__(self, formula):
        self.formula = formula
        self.items = []
        # Used to keep track of arrays, functions, and parentheses
        self.token_stack = []
    ...
    def check_scientific_notation(self):
        ...
        """
        curr_char = self.formula[self.offset]
        re_result = self.SN_RE.match("".join(self.token))
        if curr_char in "+-" and len(self.token) >= 1 and re_result:
            self.token.append(curr_char)
            self.offset += 1
            return True
        return False
