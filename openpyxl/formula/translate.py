...
class Translator:
    ...
    def translate_range(cls, range_str, rdelta, cdelta):
        ...
        if match is not None:
            result = ws_part
            result += cls.translate_row(match.group(1), rdelta)
            result += ":"
            result += cls.translate_row(match.group(2), rdelta)
            return result
        match = cls.COL_RANGE_RE.match(range_str)  # e.g. `A:BC`
        if match is not None:
            result = ws_part
            result += cls.translate_col(match.group(1), cdelta)
            result += ":"
            result += cls.translate_col(match.group(2), cdelta)
            return result
