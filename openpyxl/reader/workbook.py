...
class WorkbookParser:
    ...
    def rels(self):
        if self._rels is None:
            path = get_rels_path(self.workbook_part_name)
            self._rels = get_dependents(self.archive, path).to_dict()
        return self._rels
    ...
    def find_sheets(self):
        ...
        for sheet in self.sheets:
            if not sheet.id:
                msg = (
                    "File contains an invalid specification "
                    f"for {sheet.name}. This will be removed"
                )
                warnings.warn(msg)
                continue
    ...
    def assign_names(self):
        ...
        for idx, names in self.defined_names.by_sheet().items():
            ...
            try:
                ...
                warnings.warn(f"Defined names for sheet index {idx} cannot be located")
                continue
            for name, defn in names.items():
                ...
                if reserved is None:
                    ...
                    try:
                        ...
                        msg = (
                            "Print area cannot be set to "
                            f"Defined name: {defn.value}."
                        )
                        warnings.warn(msg)
                        continue
