...
class TestWorkbookParser:
    def test_ctor(self, datadir, workbook_parser):
        datadir.chdir()
        archive = zipfile.ZipFile("bug137.xlsx")
        parser = workbook_parser(archive, ARC_WORKBOOK)
        assert parser.archive is archive
        assert parser.sheets == []

    def test_parse_calendar(self, datadir, workbook_parser):
        datadir.chdir()
    ...
    def test_find_sheets(self, datadir, workbook_parser):
        datadir.chdir()
        archive = zipfile.ZipFile("bug137.xlsx")
        parser = workbook_parser(archive, ARC_WORKBOOK)
        parser.parse()
    ...
    def test_broken_sheet_ref(self, datadir, recwarn, workbook_parser):
    ...
    def test_print_area_title(self, datadir, workbook_parser):
        datadir.chdir()
        archive = zipfile.ZipFile("print_settings.xlsx")
        parser = workbook_parser(archive, ARC_WORKBOOK)
        parser.parse()
        wb = parser.wb
    ...
    def test_assign_names(self, datadir, workbook_parser):
    ...
    def test_name_invalid_index(self, datadir, workbook_parser, recwarn):
        datadir.chdir()
        archive = zipfile.ZipFile("print_settings.xlsx")
        parser = workbook_parser(archive, ARC_WORKBOOK)
        parser.parse()
        wb = parser.wb
        parser.defined_names.definedName = [
            DefinedName(
                name="_xlnm.Print_Area",
                localSheetId="19",
                attr_text="'New Monthly Metals'!$B$1:$O$15",
            )
        ]
        parser.assign_names()
        assert recwarn.pop().category == UserWarning

    def test_no_links(self, datadir, workbook_parser):
        datadir.chdir()
        archive = zipfile.ZipFile(io.BytesIO(), "a")
    ...
    def test_book_views(self, datadir, workbook_parser):
        datadir.chdir()
        archive = zipfile.ZipFile("bug137.xlsx")
        parser = workbook_parser(archive, ARC_WORKBOOK)
        parser.parse()
    ...
    def test_workbook_security(self, datadir, workbook_parser):
        expected_protection = WorkbookProtection()
        expected_protection.workbookPassword = "test"
        expected_protection.lockStructure = True
        datadir.chdir()
        archive = zipfile.ZipFile("workbook_security.xlsx")
        parser = workbook_parser(archive, ARC_WORKBOOK)
        parser.parse()
        assert parser.wb.security == expected_protection

    def test_defined_names_print_area(self, datadir, workbook_parser, recwarn):
        datadir.chdir()
        archive = zipfile.ZipFile("print_area_table_defined_name.xlsx")
        parser = workbook_parser(archive, ARC_WORKBOOK)
        parser.parse()
        parser.assign_names()
        assert recwarn.pop().category == UserWarning
