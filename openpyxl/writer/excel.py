...
class ExcelWriter:
    ...
    def write_data(self):
        ...
        if len(self.workbook.custom_doc_props) >= 1:
            s = tostring(self.workbook.custom_doc_props.to_tree())
            archive.writestr(ARC_CUSTOM, s)
    ...
    def write_charts(self):
        ...
        if len(self._charts) != len(set(self._charts)):
            msg = "The same chart cannot be used in more than one worksheet"
...
def save_workbook(workbook, filename):
    ...
    # warn()
    archive = zipfile.ZipFile(filename, "w", zipfile.ZIP_DEFLATED, allowZip64=True)
    tz = datetime.timezone.utc
    workbook.properties.modified = datetime.datetime.now(tz=tz).replace(tzinfo=None)
