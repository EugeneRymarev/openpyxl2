...
ARC_VOLATILE_DEPENDENCIES = f"{PACKAGE_XL}/volatileDependencies.xml"
ARC_CONNECTIONS = f"{PACKAGE_XL}/connections.xml"
## namespaces
# XML
...
REL_NS = f"{DOC_NS}relationships"
COMMENTS_REL = f"{REL_NS}/comments"
IMAGE_NS = f"{REL_NS}/image"
VML_NS = f"{REL_NS}/vmlDrawing"
VTYPES_NS = f"{DOC_NS}docPropsVTypes"
XPROPS_NS = f"{DOC_NS}extended-properties"
CUSTPROPS_NS = f"{DOC_NS}custom-properties"
EXTERNAL_LINK_NS = f"{REL_NS}/externalLink"
...
PKG_REL_NS = f"{PKG_NS}relationships"
COREPROPS_NS = f"{PKG_NS}metadata/core-properties"
CONTYPES_NS = f"{PKG_NS}content-types"
...
SPREADSHEET = "application/vnd.openxmlformats-officedocument.spreadsheetml.{}+xml"
SHARED_STRINGS = SPREADSHEET.format("sharedStrings")
EXTERNAL_LINK = SPREADSHEET.format("externalLink")
WORKSHEET_TYPE = SPREADSHEET.format("worksheet")
COMMENTS_TYPE = SPREADSHEET.format("comments")
STYLES_TYPE = SPREADSHEET.format("styles")
CHARTSHEET_TYPE = SPREADSHEET.format("chartsheet")
