...
def get_active_sheet(wb):
    ...
    """
    sheets = enumerate(wb._sheets)
    visible_sheets = [idx for idx, sheet in sheets if sheet.sheet_state == "visible"]
        ...
        for idx, sheet in enumerate(self.wb._sheets, 1):
            ...
            if not sheet.sheet_state == "visible":
                if len(self.wb._sheets) == 1:
                    msg = "The only worksheet of a workbook cannot be hidden"
