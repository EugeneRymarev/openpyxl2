...
def avoid_duplicate_name(names, value):
    ...
    if match:
        names = ",".join(names)
        pattern = f"(?P<title>{re.escape(value)})(?P<count>\\d*),?"
        sheet_title_regex = re.compile(pattern, re.I)
...
class _WorkbookChild:
    ...
    def title(self, value):
        ...
        if len(value) > 31:
            msg = "Title is more than 31 characters. Some applications may not be able to read the file"
            warnings.warn(msg)
