...
class TestSerialisable:
    ...
    def test_str(self, Immutable):
        d = Immutable()
        s = """<openpyxl.descriptors.tests.test_serialisable.Immutable object>
Parameters:
value=None"""
        assert str(d) == s

        d2 = Immutable("hello")
        s2 = """<openpyxl.descriptors.tests.test_serialisable.Immutable object>
Parameters:
value='hello'"""
        assert str(d2) == s2
...
class TestRelation:
    ...
    def test_binding(self, Relation):
        ...
        assert Relation.__namespaced__ == r
