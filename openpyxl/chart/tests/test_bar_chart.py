...
class TestBarChart:
    ...
    def test_from_tree(self, bar_chart):
        src = """
        <barChart>
            <barDir val="col"/>
            <grouping val="clustered"/>
        ...
        """
        node = fromstring(src)
        bc = bar_chart.from_tree(node)
        assert bc == bar_chart(varyColors=False, axId=(10, 100))
        assert bc.axId == [10, 100]
        assert bc.grouping == "clustered"

    def test_write(self, bar_chart):
        chart = bar_chart()
    ...
    def test_series(self, bar_chart):
        from openpyxl.chart.series_factory import SeriesFactory

        s1 = SeriesFactory(values="Sheet1!$A$1:$A$10")
        s2 = SeriesFactory(values="Sheet1!$B$1:$B$10")
        bc = bar_chart(ser=[s1, s2])
        xml = tostring(bc.to_tree())
        expected = """
        <barChart>
            <barDir val="col"></barDir>
...
class TestBarChart3D:
    def test_ctor(self, bar_chart_3d):
        bc = bar_chart_3d()
    ...
    def test_from_xml(self, bar_chart_3d):
        src = """
        <bar3DChart>
            <barDir val="col"/>
            <grouping val="clustered"/>
        ...
        """
        node = fromstring(src)
        bc = bar_chart_3d.from_tree(node)
